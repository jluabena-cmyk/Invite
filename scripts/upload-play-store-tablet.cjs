#!/usr/bin/env node
/**
 * upload-play-store-tablet.cjs
 *
 * Uploads Android 7-inch tablet screenshots to the Google Play Store listing
 ...
 *
 * Usage:
 *   node scripts/upload-play-store-tablet.cjs [--language en-US]
 *
 * What it does:
 *   1. Creates a new draft edit for the app.
 *   2. Deletes all existing sevenInchScreenshots for the listing language
 *      so the new set fully replaces the old one.
 *   3. Uploads every .png file in screenshots/android-tablet/ in sorted order.
 ...
 *
 * Screenshots directory: screenshots/android-tablet/
 * Target image type:     sevenInchScreenshots  (1200×1920 px)
 */
...
const SCREENSHOT_DIR = path.resolve(__dirname, "../screenshots/android-tablet");
const IMAGE_TYPE = "sevenInchScreenshots";
...
function preflight() {
  ...
  if (!fs.existsSync(SCREENSHOT_DIR)) {
    ...
    console.error("Run scripts/capture-screenshots-tablet.cjs first.");
    process.exit(1);
  }
...
async function main() {
  ...
  try {
    // 2. Clear existing sevenInchScreenshots for this language so the new set
  ...
  console.log(
    `\n✓ All ${files.length} tablet screenshots uploaded to the Play Store listing.`
  );
  console.log(
